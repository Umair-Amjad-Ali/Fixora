...
const TAB_OPTIONS = [
  { id: "all", labelKey: "tabs.all", icon: Package },
  { id: "pending", labelKey: "tabs.pending", icon: Clock },
  { id: "completed", labelKey: "tabs.completed", icon: CheckCircle2 },
  { id: "guide", labelKey: "tabs.guide", icon: BookOpen },
];
...
export default function ProfileDashboard() {
  ...
  if (loading || !user) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-zinc-950">
        ...
        <p className="text-zinc-500 font-black uppercase tracking-widest text-[10px]">{t("verifyingIdentity")}</p>
      </div>
    );
  }
  ...
  const StatusBadge = ({ status }: { status: string }) => {
    switch (status) {
      ...
      case "in_progress":
        return <span className="px-3 py-1 bg-blue-500/10 text-blue-500 border border-blue-500/20 rounded-lg text-xs font-black uppercase tracking-tight">{t("status.in_progress")}</span>;
      case "completed":
        return <span className="px-3 py-1 bg-emerald-500/10 text-emerald-500 border border-emerald-500/20 rounded-lg text-xs font-black uppercase tracking-tight">{t("status.completed")}</span>;
      case "cancelled":
      ...
      <div className="pt-24 pb-20 bg-zinc-950 overflow-hidden relative border-b border-white/5">
        ...
        <div className="container-tight relative z-10">
          ...
          >
            ...
            <span className="text-[10px] font-black uppercase tracking-[0.2em]">{tCommon("backToHome")}</span>
          </Link>
              ...
              >
                ...
                <div className="bg-primary text-white rounded-3xl p-8 relative overflow-hidden shadow-2xl shadow-primary/20">
                  ...
                  <div className="relative z-10">
                    <h2 className="text-2xl font-black tracking-tight mb-2">{t("guide.bannerTitle")}</h2>
                    <p className="text-white/80 font-bold text-sm max-w-md">{t("guide.bannerSubtitle")}</p>
                  </div>
                </div>
                ...
                <div className="flex justify-center pt-6">
                  ...
                  >
                    {t("findService")} <ArrowRight size={18} className="group-hover:translate-x-1 rtl:group-hover:-translate-x-1 rtl:rotate-180 transition-transform" />
                  </button>
                </div>
              </motion.div>
            ) : (
              ...
              >
                {fetching ? (
                  ...
                  <>
                    <div className="space-y-4">
                      {paginatedOrders.map((order) => {
                        ...
                        const dateDesc = dateStr ? format(new Date(dateStr), "MMM dd, yyyy", { locale: activeLocale }) : tCommon("tbd");
                        ...
                        return (
                          ...
                          >
                            <div className="flex-1 w-full">
                               <div className="flex justify-between items-start mb-4">
                                  ...
                                  <div className="flex flex-col items-end gap-1">
                                    ...
                                    <span className="text-[8px] font-black text-zinc-500 uppercase tracking-tighter bg-zinc-50 dark:bg-slate-800 px-1.5 py-0.5 rounded border border-zinc-100 dark:border-slate-800">
                                      {t("booked")}: {order.createdAt?.seconds ? format(new Date(order.createdAt.seconds * 1000), "MMM dd", { locale: activeLocale }) : tCommon("tbd")}
                                    </span>
                                  </div>
                               </div>
                               <h3 className="text-xl sm:text-2xl font-black text-foreground mb-1 capitalize tracking-tight">
                                  {tServices(`${order.service?.serviceType}.name`)} {t("service")}
                               </h3>
                               ...
                               <div className="flex flex-wrap gap-3">
                                  ...
                                  <div className="flex items-center gap-2 bg-zinc-50 dark:bg-slate-800 px-3 py-2 rounded-xl border border-zinc-100 dark:border-slate-700 text-[11px] font-black text-zinc-600 dark:text-zinc-300">
                                     ...
                                     {order.schedule?.timeRange || tCommon("tbd")}
                                  </div>
                            ...
                            <div className="w-full sm:w-auto shrink-0">
                               ...
                               >
                                  <ArrowRight size={20} className="group-hover:translate-x-0.5 rtl:group-hover:-translate-x-0.5 rtl:rotate-180 transition-transform" />
                               </button>
                            </div>
                          </motion.div>
                        );
                  ...
                  <div className="bg-white dark:bg-slate-900/50 border-2 border-dashed border-zinc-200 dark:border-slate-800 rounded-3xl p-16 text-center shadow-sm">
                    ...
                    <h3 className="text-2xl font-black text-foreground mb-2">{t("noOrders")}</h3>
                    <p className="text-sm text-zinc-500 font-medium max-w-sm mx-auto mb-10 leading-relaxed">
                      {filterDate 
                        ? t("noOrdersDescDate", { date: format(new Date(filterDate), "MMM dd, yyyy", { locale: activeLocale }) }) 
                        : t("noOrdersDesc")}
                    </p>
                    <button 
                      onClick={() => {
                    ...
                    >
                      {filterDate ? t("clearFilters") : t("findService")} <ArrowRight size={14} className="rtl:rotate-180" />
                    </button>
                  </div>
                )}
              </motion.div>
