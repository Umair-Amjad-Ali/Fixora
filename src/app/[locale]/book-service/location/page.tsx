...
const LoadingMap = () => {
  ...
  return (
    <div className="w-full h-full flex flex-col items-center justify-center bg-zinc-100 dark:bg-slate-900 rounded-[2.5rem]">
      ...
      <span className="text-sm font-bold text-zinc-500 uppercase tracking-widest">{t("loadingMap")}</span>
    </div>
  );
};

// Dynamically import Google Maps component to prevent SSR errors and improve initial bundle size
const MapComponent = dynamic(() => import("@/components/booking/MapComponent"), { ssr: false, loading: () => <LoadingMap /> });
...
export default function LocationPage() {
  ...
  const reverseGeocode = async (lat: number, lng: number) => {
    try {
      ...
      if (data) {
        const { formattedAddress, city, area } = LocationService.extractAddressDetails(data, DEFAULT_CENTER.label);
  ...
  const handleSearch = async (e: React.FormEvent) => {
    ...
    setIsSearching(true);
    try {
      const results = await LocationService.searchLocation(searchQuery, activeCountry, mapLanguage);
  ...
  const selectSearchResult = (result: any) => {
    ...
    const { formattedAddress, city, area } = LocationService.extractAddressDetails(result, DEFAULT_CENTER.label);
  ...
  const getUserLocation = async () => {
    try {
      toast.loading(t("findingLocation"));
      ...
      <div className="mb-8 text-start">
        ...
        <div className="flex items-center justify-between">
            ...
            <div className="hidden md:flex items-center gap-2 text-[10px] font-black uppercase tracking-widest text-primary bg-primary/10 px-3 py-1.5 rounded-lg border border-primary/20">
              <ShieldCheck size={12} /> {t("openStreetMapSecure")}
            </div>
        </div>
      </div>
            ...
            <div className="relative z-50">
              <div className="relative group">
                <button 
                  ...
                  className="absolute left-4 rtl:left-auto rtl:right-4 top-1/2 -translate-y-1/2 text-zinc-400 group-focus-within:text-primary transition-colors hover:scale-110"
                >
                ...
                <input
                  ...
                  placeholder={mapLanguage === "en" ? t("searchPlaceholder") : t("searchPlaceholderAr")}
                  className="w-full h-11 pl-11 pr-40 rtl:pr-11 rtl:pl-40 bg-white dark:bg-slate-950 border border-zinc-200 dark:border-slate-800 rounded-xl shadow-lg outline-none focus:border-primary transition-all font-medium text-sm"
                />
                ...
                <div className="absolute right-2 rtl:right-auto rtl:left-2 top-1/2 -translate-y-1/2 flex items-center gap-1">
                  ...
                  >
                    <Crosshair size={12} /> <span className="hidden sm:inline">{t("locate")}</span>
                  </Button>
                </div>
              </div>
            ...
            <div className="bg-zinc-50 dark:bg-slate-900/40 p-6 rounded-3xl border border-zinc-200 dark:border-slate-800 space-y-5 h-full flex flex-col justify-between">
              <div className="space-y-5">
                ...
                <div className="space-y-1.5">
                  <label className="text-[10px] font-black uppercase tracking-widest text-zinc-400">{t("detectedArea")}</label>
                ...
                <div className="space-y-1.5">
                  ...
                  {errors.formattedAddress && (
                    <p className="text-red-500 text-[10px] font-bold mt-1 text-start">{t("addressError")}</p>
                  )}
                </div>
                ...
                <div className="space-y-1.5">
                  ...
                  {errors.buildingDetails && (
                    <p className="text-red-500 text-[10px] font-bold mt-1 text-start">{t("buildingError")}</p>
                  )}
                </div>
              </div>
              ...
              <div className="pt-5 border-t border-zinc-200 dark:border-slate-800">
                <div className="flex items-center gap-3 p-3.5 bg-primary/5 border border-primary/20 rounded-2xl">
                   ...
                   <p className="text-[11px] font-medium text-zinc-600 dark:text-zinc-400 leading-relaxed">
                     {t("coordinatesShared", { lat: markerPos.lat.toFixed(4), lng: markerPos.lng.toFixed(4) })}
                   </p>
                </div>
              </div>
            </div>
