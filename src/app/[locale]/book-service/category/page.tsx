  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6">
      <div className="mb-10 text-center">
        ...
        >
          <Sparkles size={12} />
          {t('categoryPage.stepBadge')}
        </motion.div>
        <h2 className="text-3xl font-black mb-2 text-foreground tracking-tight">{t('categoryPage.title')}</h2>
        <p className="text-zinc-500 font-medium text-sm">{t('categoryPage.subtitle')}</p>
      </div>
        ...
        {SERVICE_CATEGORIES.map((category) => (
          ...
          >
            ...
            <h3 className="relative z-10 text-[16px] font-black text-foreground group-hover:text-primary transition-colors leading-tight mb-1.5">
              {t(`constants.services.${category.slug}.name`)}
            </h3>
            ...
            <p className="relative z-10 text-[11px] text-zinc-500 font-medium mb-4 line-clamp-2 px-2 opacity-80 group-hover:opacity-100 transition-opacity">
              {t(`constants.services.${category.slug}.description`)}
            </p>
            ...
            {category.startingPrice && (
              <div className="relative z-10 mt-auto pt-3 border-t border-zinc-100 dark:border-slate-800 w-full flex flex-col items-center">
                <span className="text-[8px] font-black uppercase tracking-widest text-zinc-400 mb-0.5">{t('serviceTypePage.startingFrom')}</span>
            ...
            <div className="absolute bottom-4 right-4 translate-x-4 opacity-0 group-hover:opacity-100 group-hover:translate-x-0 transition-all duration-300 font-bold rtl:left-4 rtl:right-auto rtl:-translate-x-4 rtl:group-hover:translate-x-0">
